...
def fastrcnn_loss(class_logits, box_regression, labels, regression_targets):
    ...
    classification_loss = F.cross_entropy(class_logits, labels)#,weight=torch.tensor([0.2,0.3,0.5]).cuda())
...
class Decouple_RoIHeads(torch.nn.Module):
    __annotations__ = {
    ...
    def __init__(self,
        ...
        super(Decouple_RoIHeads, self).__init__()
        self.conv = nn.Conv2d(input_channel,out_channel,kernel_size=1)
    ...
    def forward(self,
        ...
        # HCE 生成context特征方式:使用类似non-local方式进行全局特征的融合
        whole = []
        for i, img_shape in zip(proposals, image_shapes):
            j = torch.tensor([[0,0,img_shape[0],img_shape[1]]],dtype=torch.float32).cuda()
            # j = i.clone()
            # j[:, 0] = 0  # x_min
            # j[:, 1] = 0  # y_min
            ...
            whole.append(j)
        box_features_global = self.conv(self.box_roi_pool_context(x_context, whole, image_shapes))

        box_features_fusion = self.Attention_global(box_features_fpn, box_features_global)
        box_features = self.box_head(box_features_fpn)
        box_regression = self.box_predictor(box_features)

        class_features = self.class_head(box_features_fusion)
