...
if __name__ == '__main__':
    ...
    for epoch in range(args.epochs):
        ...
        for i, data in enumerate(train_dataloader):
            ...
            total_losses = losses_dict['img_level_loss']+losses_dict['loss_classifier'] + losses_dict['loss_box_reg'] + losses_dict[
            ...
            print('Train: [{}:{}/{}]: train_losses：{},img_level_loss:{} '
                  ...
                  'loss_objectness:{},loss_rpn_box_reg:{}'.format(epoch,i,len(train_dataloader),
                                               ...
                                               losses_dict['loss_classifier'].item(), losses_dict["loss_box_reg"].item(),
        ...
        f.write('Epoch_{},\ttrain_average_loss:  {},\ttrain_average_img_level_loss:{},'
                '\ttrain_average_loss_classifier:  {},\ttrain_average_loss_boxreg:  {},'
                '\ttrain_average_loss_objectness:  {},\ttrain_average_loss_rpnboxreg:  {}\n'.format(
            epoch,train_average_loss,train_average_img_level_loss,train_average_loss_classifier,train_average_loss_boxreg,train_average_loss_objectness,train_average_loss_rpnboxreg))
        ...
        for j, data in enumerate(valid_dataloader):
            ...
            inputs = list(input.to(device) for input in inputs)
            labels = [{k: v.to(device) for k, v in t.items()} for t in labels]#[{},{}...]
            outputs,img_level_label = model(inputs)
            ...
            for l, p in zip(labels, img_level_label):
                valid_img_labels.append(l['labels'].item())
                try:
                    valid_img_preds.append(p.item())
                except ValueError:
                    if l['labels'].item() == 1:
                        valid_img_preds.append(2)
                    elif l['labels'].item()==2:
                        valid_img_preds.append(3)
                    else:
                        valid_img_preds.append(1)
        ...
        valid_acc_img = accuracy_score(valid_img_labels,valid_img_preds)

        writer.add_scalar('valid_acc', valid_acc, epoch)
        writer.add_scalar('valid_acc_img', valid_acc_img, epoch)
        ...
        f.write('valid metrics:\tap_1:  {},\tap_2:  {},\tap_3:  {},\tmap:  {}\tvalid acc:{}\tvalid acc img_brain:{}\n\n'.format(ap_1,ap_2,ap_3,map,valid_acc,valid_acc_img))
        ...
        for s, data in enumerate(test_dataloader):
            ...
            for l, p in zip(labels, img_level_label):
                test_img_labels.append(l['labels'].item())
                try:
                    test_img_preds.append(p.item())
                except ValueError:
                    if l['labels'].item() == 1:
                        test_img_preds.append(2)
                    elif l['labels'].item() == 2:
                        test_img_preds.append(3)
                    else:
                        test_img_preds.append(1)
        ...
        test_img_acc = accuracy_score(test_img_labels, test_img_preds)
        f.write('test metrics:\tap_1:  {},\tap_2:  {},\tap_3:  {},\tmap:  {}\t test acc:{}\t test acc img_brain:{}\n'.format(test_ap_1,
                                                                                                        test_ap_2,
                                                                                                        test_ap_3,
                                                                                                        test_map,
                                                                                                        test_acc,
        ...
        if map > best_map or test_map > 0.78:
            if map>best_map:
                best_map = map
            checkpoint = str(epoch) + "_best_" + str(map)+'_'+str(valid_acc)+'_'+str(valid_acc_img)+'_'+str(test_map)+'_'+str(test_acc)+'_'+str(test_img_acc)
