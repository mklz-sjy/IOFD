...
if __name__ == '__main__':
    ...
    #settings
    #HCE train and test mode
    parser.add_argument('--train_mode', help='choose one of mode: before, after or all', type=str,required=True)
    parser.add_argument('--test_mode', help='choose one of mode: before, after or all', type=str,required=True)
    ...
    args.task_name = '{}/BrainTumor Model_{} Bs_{} Lr_{} Epoch_{} LrDrop_{}'.format(args.output_path, model_name,
                                                                                    args.batch_size, args.lr,
                                                                                    args.epochs, args.lr_drop)
    ...
    for epoch in range(args.epochs):
        train_losses = 0
        train_total_img_level_loss = 0
        train_total_loss_classifier_before = 0
        train_total_loss_boxreg_before = 0
        train_total_loss_classifier_after = 0
        train_total_loss_boxreg_after = 0
        ...
        for i, data in enumerate(train_dataloader):
            ...
            if args.train_mode == 'before':
                total_losses = losses_dict['img_level_loss']+losses_dict['loss_classifier_before'] + losses_dict['loss_box_reg_before'] + losses_dict[
                    'loss_objectness'] + losses_dict['loss_rpn_box_reg']
            elif args.train_mode == 'after':
                total_losses = losses_dict['img_level_loss']+losses_dict['loss_classifier_after'] + losses_dict['loss_box_reg_after'] + losses_dict[
                    'loss_objectness'] + losses_dict['loss_rpn_box_reg']
            elif args.train_mode == 'all':
                total_losses = losses_dict['img_level_loss']+losses_dict['loss_classifier_before'] + losses_dict['loss_box_reg_before'] +losses_dict['loss_classifier_after'] + losses_dict['loss_box_reg_after'] + losses_dict[
                    'loss_objectness'] + losses_dict['loss_rpn_box_reg']
            else:
                assert False,'Train Mode Error'
            ...
            if args.train_mode == 'before':
                train_total_loss_classifier_before += losses_dict['loss_classifier_before'].item()
                train_total_loss_boxreg_before += losses_dict['loss_box_reg_before'].item()
            elif args.train_mode == 'after':
                train_total_loss_classifier_after += losses_dict['loss_classifier_after'].item()
                train_total_loss_boxreg_after += losses_dict['loss_box_reg_after'].item()
            elif args.train_mode == 'all':
                train_total_loss_classifier_before += losses_dict['loss_classifier_before'].item()
                train_total_loss_boxreg_before += losses_dict['loss_box_reg_before'].item()
                train_total_loss_classifier_after += losses_dict['loss_classifier_after'].item()
            ...
            train_total_img_level_loss += losses_dict['img_level_loss'].item()

            print('Train: [{}:{}/{}]: train_losses：{},img_level_loss:{} '
                  'loss_classifier_before:{}, loss_box_reg_before:{},'
                  'loss_classifier_after:{}, loss_box_reg_after:{}, '
                  'loss_objectness:{},loss_rpn_box_reg:{}'.format(epoch,i,len(train_dataloader),
                                               total_losses.item(),losses_dict['img_level_loss'].item(),
                                               losses_dict['loss_classifier_before'].item(), losses_dict["loss_box_reg_before"].item(),
                                               losses_dict['loss_classifier_after'].item(), losses_dict["loss_box_reg_after"].item(),
                                               losses_dict["loss_objectness"].item(),losses_dict["loss_rpn_box_reg"].item()))
        lr_scheduler.step()

        train_average_loss=train_losses/(i+1)
        train_average_img_level_loss = train_total_img_level_loss/(i+1)
        train_average_loss_classifier_before = train_total_loss_classifier_before / (i + 1)
        train_average_loss_boxreg_before = train_total_loss_boxreg_before / (i + 1)
        train_average_loss_classifier_after = train_total_loss_classifier_after/(i+1)
        train_average_loss_boxreg_after = train_total_loss_boxreg_after/(i+1)
        train_average_loss_objectness = train_total_loss_objectness/(i+1)
        train_average_loss_rpnboxreg = train_total_loss_rpnboxreg/(i+1)


        f.write('Epoch_{},\ttrain_average_loss:  {},\ttrain_average_img_level_loss:{},'
                '\ttrain_average_loss_classifier_before:  {},\ttrain_average_loss_boxreg_before:  {},'
                '\ttrain_average_loss_classifier_after:  {},\ttrain_average_loss_boxreg_after:  {},'
                '\ttrain_average_loss_objectness:  {},\ttrain_average_loss_rpnboxreg:  {}\n'.format(
            epoch,train_average_loss,train_average_img_level_loss,train_average_loss_classifier_before,train_average_loss_boxreg_before,train_average_loss_classifier_after,train_average_loss_boxreg_after,train_average_loss_objectness,train_average_loss_rpnboxreg))
        writer.add_scalar('train_loss', train_average_loss, epoch)
        writer.add_scalar('train_average_img_level_loss',train_average_img_level_loss,epoch)
        writer.add_scalar('train_average_loss_classifier_before', train_average_loss_classifier_before, epoch)
        writer.add_scalar('train_average_loss_boxreg_before', train_average_loss_boxreg_before, epoch)
        writer.add_scalar('train_average_loss_classifier_after', train_average_loss_classifier_after, epoch)
        writer.add_scalar('train_average_loss_boxreg_after', train_average_loss_boxreg_after, epoch)
        ...
        f.write('valid metrics:\tap_1:  {},\tap_2:  {},\tap_3:  {},\tmap:  {}\tvalid_acc:{}\n\n'.format(ap_1,ap_2,ap_3,map,valid_acc))
        print('valid metrics:\tap_1:  {},\tap_2:  {},\tap_3:  {},\tmap:  {}\tvalid_acc:{}\n\n'.format(ap_1, ap_2, ap_3, map,valid_acc))
        ...
        f.write('test metrics:\tap_1:  {},\tap_2:  {},\tap_3:  {},\tmap:  {}\ttest_acc:{}\n'.format(test_ap_1,
                                                                                                        ...
                                                                                                        test_ap_3,
                                                                                                        test_map,test_acc))
        if map > best_map or test_map>0.78:
            if map>best_map:
                best_map = map
